...
Install a packed artifact, registry version, or user-approved Git spec through DSH's profile package manager. Do not use a repository-relative `link:` or `file:` spec; the package must first prove that its own artifact is complete. Before installation, enumerate every required peer in the package manifest and verify that the target profile already provides each one; for a Stent-dependent package, that includes `@oh-my-dsh/stent` and, when imported, `@oh-my-dsh/stent-api` at the package's required range. Install `@oh-my-dsh/stent-pack` first rather than adding a second Stent copy to the consumer. The package repository's peer policy is not transferred to the consuming profile; do not claim that `dsh plugin add` rejects a missing peer unless the profile's own peer policy has been independently verified.
