...
2. **`fabric-dsh` loads it** — the boot succeeds, the plugin appears in the client manifest, and the required patch recorded a binding (the post-boot check fails the boot when it did not).
